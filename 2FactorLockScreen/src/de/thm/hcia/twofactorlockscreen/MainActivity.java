...
public class MainActivity extends SlidingFragmentActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// get version number
		try {
			mAppVersion = getPackageManager().getPackageInfo(getPackageName(),
					0).versionName;
		...
		if (findViewById(R.id.menu_frame) == null) {
			...
			/*
			 * add a dummy view disable SlidingMenu
			 */
		...
		if (savedInstanceState == null
				&& sPiO.getBoolean(SharedPreferenceIO.PREF_START_INFO_READ) == false) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case MainActivity.REQ_CODE_CREATE_PATTERN:
			if (resultCode == RESULT_OK) {
				 ...
				 */
				Toast.makeText(mContext, R.string.pattern_recorded,
		...
		case REQ_CODE_COMPARE_PATTERN:
			...
			// {
			// Toast.makeText(mContext, R.string.speech_recorded,
			// Toast.LENGTH_SHORT).show();
			// // Log.d("TEST",
			// loadArrayFromSharedPreferences("savedRecord").toString());
			// }else{
			// Toast.makeText(mContext, R.string.writing_prefs_error_01,
			// Toast.LENGTH_SHORT).show();
			// }
			//
			// }
			...
			// Toast.makeText(this, R.string.voice_no_match,
			// Toast.LENGTH_LONG).show();
			// }
			// break;
		}
	...
	private void showExplanationDialog() {
		...
		mDontShowAgain = (CheckBox) checkBoxLayout
				.findViewById(R.id.never_again);
				...
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						if (mDontShowAgain.isChecked()) {
							sPiO.putBoolean(
									SharedPreferenceIO.PREF_START_INFO_READ,
									true);
						}
					}
				}).show();
